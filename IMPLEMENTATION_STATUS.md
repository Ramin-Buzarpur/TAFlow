...
| Talent pool search/filter | COMPLETE | `src/app/talent-pool/page.tsx`, `src/app/api/talent-pool/route.ts`, and `src/server/services/ta-workflow.ts` now provide search, status filtering, sort modes, and role-gated access; `tests/e2e/talent-pool.spec.ts` covers professor happy path and student denial. |
| Survey results dashboard/export | COMPLETE | `src/app/surveys/ui.tsx`, `src/server/services/surveys.ts`, and `src/app/api/surveys/[id]/export/route.ts` now provide privacy-threshold gated summaries and CSV export; `tests/e2e/survey-results.spec.ts` covers hidden/visible/export flow. |
...
| Grade score upper bound | COMPLETE | A Postgres trigger now enforces `GradeRecord.score <= GradeItem.maxScore` on insert/update and blocks lowering a grade item's `maxScore` beneath existing records. Covered by `pnpm test:integration`. |

...
export async function deleteCourseMaterial(actorId: string, materialId: string) {
  ...
  await requireCoursePermission(actorId, material.courseOfferingId, coursePermissions.MANAGE_COURSE_MATERIALS);
  await db.courseMaterial.delete({ where: { id: materialId } });
  await writeAuditLog({ actorId, action: "DELETE", entityType: "CourseMaterial", entityId: materialId, courseOfferingId: material.courseOfferingId });

...
export function SurveyBuilder({ offerings }: { offerings: { id: string; title: string }[] }) {
  ...
  async function submit(fd: FormData) {
    ...
    setMsg(res.ok ? `نظرسنجی ساخته شد (شناسه: ${j.id})` : j.message || "خطا در ساخت نظرسنجی");
  }
  return <form className="form" action={submit}>
    ...
    <input className="input" name="title" placeholder="عنوان نظرسنجی" required/>
    ...
    <button className="btn btn-primary">انتشار نظرسنجی</button>
...
export function SurveyResultsViewer() {
  ...
  return <div className="stack">
    <form className="form" action={load}><input className="input" name="surveyId" placeholder="شناسه نظرسنجی" required/><button className="btn">مشاهده نتایج</button></form>
    {msg ? <p className="muted">{msg}</p> : null}

...
export default async function ProfessorEvaluationPage() {
  ...
  if (!session?.user?.id) return <><Topbar/><main className="shell"><EmptyState title="ورود لازم است" text="برای ارزشیابی وارد شوید."/></main></>;
  const offerings = await listEvaluableCourseOfferings(session.user.id);
  return <><Topbar/><main className="shell">
    <div className="page-title"><div><h1>ارزشیابی</h1><p className="muted">فقط دانشجویان ثبت‌نام‌شده در درس می‌توانند ارزیابی کنند؛ پاسخ‌ها ناشناس ثبت می‌شوند و هر دانشجو فقط یک بار می‌تواند ارزیابی کند.</p></div></div>

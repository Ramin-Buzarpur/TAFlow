...
import { createSurveyFull, listSurveys } from "@/server/services/survey-full";
...
export async function POST(request: Request) { try { const user = await requireUser(); const body = createSurveySchema.parse(await request.json()); return created(await createSurveyFull(user.id, body)); } catch (e) { return fail(e, "Could not create survey"); } }

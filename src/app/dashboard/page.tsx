...
export default async function DashboardPage() {
  ...
  return <><Topbar/><CommandPalette/><main className="shell">
    ...
    {isHeadTa ? <><h2 style={{ marginTop: 30 }}>داشبورد Head TA</h2>
      <section className="grid grid-4"><Kpi label="درس‌های تحت مدیریت" value={headTa.counters.managedCourses}/><Kpi label="تعداد تیم TA" value={headTa.counters.teamSize} tone="purple"/><Kpi label="وظایف باز" value={headTa.counters.openTasks} tone="orange"/><Kpi label="پیام‌های باز" value={headTa.counters.unansweredThreads} tone="green"/></section>
    ...
    {admin ? <><h2 style={{ marginTop: 30 }}>داشبورد ادمین</h2><section className="grid grid-4"><Kpi label="کل کاربران" value={admin.users}/><Kpi label="ارائه‌های درس" value={admin.offerings}/><Kpi label="فرصت‌های فعال" value={admin.opportunities}/><Kpi label="گواهی در انتظار" value={admin.certificates}/></section><div style={{ display:"flex", gap:10, marginTop:16 }}><Link className="btn" href="/admin">پنل مدیریت</Link><Link className="btn" href="/reports">گزارش‌ها و نمودارها</Link></div></> : null}

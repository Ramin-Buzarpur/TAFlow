...
export function Kpi({ label, value, tone = "blue" }: { label: string; value: React.ReactNode; tone?: "blue"|"green"|"orange"|"purple" }) { return <Card><div className="kpi"><div><p className="muted">{label}</p><strong>{value}</strong></div><span className={`iconbox b-${tone}`}/></div></Card>; }

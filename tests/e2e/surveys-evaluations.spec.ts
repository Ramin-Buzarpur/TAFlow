...
test.describe("professor", () => {
  ...
  test("TA survey builder creates a multi-question survey with a response threshold", async ({ page }) => {
    ...
    await page.getByRole("button", { name: "انتشار نظرسنجی" }).click();
    await expect(page.getByText(/نظرسنجی ساخته شد/)).toBeVisible({ timeout: 10_000 });
  });
});
